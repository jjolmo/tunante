...
/// The first line of a playlist this wrote.
///
/// Load-bearing rather than decorative: it is how a later run tells its own
/// output from a file somebody else put there, and only the second is worth
/// refusing to overwrite.
pub const MARKER: &str = "# Generated by Tunante from zophar.net";

/// Did this playlist come from here?
pub fn is_ours(body: &str) -> bool {
    body.lines().next().is_some_and(|l| l.trim() == MARKER)
}
...
mod tests {
    ...
    /// The distinction that decides whether replacing is a question at all.
    #[test]
    fn a_playlist_this_wrote_is_recognised_as_its_own() {
        let mine = to_m3u("x.nsf", &[Entry { number: 1, title: "A".into(), length: String::new() }]);
        assert!(is_ours(&mine));
        assert!(!is_ours("x.nsf::NSF,1,Overworld,1:30\n"), "a hand-written one is not");
        assert!(!is_ours(""), "and neither is nothing");
    }
