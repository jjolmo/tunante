...
pub trait Tray: Sized + Send + 'static {
    ...
    /// The host's XDG activation token for the activation it is about to ask
    /// for. Hosts that support it (KDE Plasma) call this right before
    /// `Activate`, so a window raised by it can hand the token to the
    /// compositor instead of being held back by focus-stealing prevention.
    ///
    /// Tunante patch: not in upstream ksni 0.3.6.
