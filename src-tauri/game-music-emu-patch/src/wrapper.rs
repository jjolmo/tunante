...
impl GameMusicEmu {
    ...
    /// Get track information (title, author, game, duration, etc.)
    pub fn track_info(&self, track: usize) -> GmeResult<native::TrackInfo> {
        native::track_info(&self.handle, track)
    }

    /// Seek to position in milliseconds
    pub fn seek(&self, msec: i32) -> GmeResult<()> {
