...
#[tauri::command]
/// Quick scan: only add files that are NOT already in the database.
/// Much faster than resync_library (which clears everything and re-scans).
pub fn quick_scan(state: State<'_, Arc<AppState>>, app: tauri::AppHandle) -> Result<(), String> {
    let state = state.inner().clone();
    let app = app.clone();
    ...
    std::thread::spawn(move || {
        let existing_paths = {
            let db = state.db.lock();
            db.get_all_track_paths().unwrap_or_default()
        };
        ...
        for folder in &folders {
            let scan_path = PathBuf::from(&folder.path);
            if !scan_path.exists() {
                continue;
            }

            let new_files: Vec<PathBuf> = WalkDir::new(&scan_path)
                .follow_links(true)
                .into_iter()
                .filter_map(|e| e.ok())
                .filter(|e| e.file_type().is_file() && is_audio_file(e.path()))
            ...
            log::info!("Quick scan: {} new files in {}", new_files.len(), folder.path);
            ...
            for file_path in &new_files {
                match metadata::read_metadata_all(file_path) {
                    ...
                    Err(e) => {
                        log::warn!("Quick scan metadata error {}: {}", file_path.display(), e);
                    }
                }
            }
        }
        ...
        if new_count > 0 {
            log::info!("Quick scan: added {} new tracks", new_count);
            let _ = app.emit("scan-complete", ());
        }
    });
