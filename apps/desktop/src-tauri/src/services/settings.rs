//! Settings, monitored folders, pinned folders, and the Linux desktop entry.
//! Bodies extracted verbatim from `commands/settings.rs`; `set_tray_visible`
//! stayed behind — it talks to the tray handle and nothing else, so it *is*
//! shell.
...
pub fn get_settings(state: &AppState) -> Result<Vec<Setting>, String> {
    state.db.lock().get_all_settings().map_err(|e| e.to_string())
}

pub fn get_setting(state: &AppState, key: &str) -> Result<Option<String>, String> {
    state.db.lock().get_setting(key).map_err(|e| e.to_string())
}

pub fn set_setting(state: &AppState, key: &str, value: &str) -> Result<(), String> {
    let result = state
        .db
        .lock()
    ...
    if key == crate::metadata::rating_source::SETTING_KEY {
        crate::services::library::reset_rating_resolution();
    }

    result
}

pub fn get_monitored_folders(state: &AppState) -> Result<Vec<MonitoredFolder>, String> {
    state
        .db
        .lock()
...
pub fn add_monitored_folder(
    ...
    std::thread::spawn(move || {
        crate::services::library::scan_folder_sync(&state_inner, &events, &scan_path);
...
pub fn remove_monitored_folder(
    ...
    let db = state.db.lock();
    db.remove_monitored_folder(id).map_err(|e| e.to_string())?;
    ...
    events.emit(AppEvent::LibraryUpdated(None));

    Ok(())
}

pub fn toggle_folder_watching(state: &AppState, id: &str, enabled: bool) -> Result<(), String> {
    state
        .db
        .lock()
...
pub fn get_pinned_folders(state: &AppState) -> Result<Vec<PinnedFolder>, String> {
    state
        .db
        .lock()
        .get_pinned_folders()
...
pub fn remove_pinned_folder(state: &AppState, events: &Events, id: &str) -> Result<(), String> {
    let db = state.db.lock();
    let pins = db.get_pinned_folders().map_err(|e| e.to_string())?;
...
pub fn get_desktop_entry_path() -> String {
    #[cfg(target_os = "linux")]
    {
        if let Some(home) = std::env::var_os("HOME") {
            let path = PathBuf::from(home).join(".local/share/applications/tunante.desktop");
...
pub fn create_desktop_entry() -> Result<String, String> {
    #[cfg(target_os = "linux")]
    {
        ...
        std::fs::write(&icon_dest, ICON_PNG).map_err(|e| format!("Cannot write icon: {}", e))?;
