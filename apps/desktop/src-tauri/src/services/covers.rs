...
//! and reporting progress through [`Events`].
//!
//! Everything here is **blocking** — a lookup can take thirty seconds. The
//! Tauri shells wrap these in `spawn_blocking`; another UI will call them from
...
fn request_for(track: &Track, store_in_folder: bool) -> CoverRequest {
    ...
    let all = all_systems();
    CoverRequest {
        libretro_system: console::by_id(&track.console_id)
...
fn data_uri(f: &tunante_art::resolver::Resolved) -> String {
    use base64::Engine;
    format!(
        "data:{};base64,{}",
        f.info.format.mime(),
...
pub fn resolve_cover(state: &AppState, track_path: &str) -> Result<Option<String>, String> {
    ...
    let found = resolver()
        .resolve_and_store(&req, Confidence::High, Overwrite::Never)
        .map_err(|e| e.to_string())?;

    Ok(found.0.map(|f| data_uri(&f)))
}
...
pub fn refetch_cover(state: &AppState, track_path: &str) -> Result<Option<String>, String> {
    let track = state
        .db
        .lock()
        .get_track_by_path(track_path)
    ...
    r.forget(&req);
    let found = r
        .resolve_and_store(&req, Confidence::Low, Overwrite::Replace)
        .map_err(|e| e.to_string())?;
...
pub fn choose_cover(state: &AppState, track_path: &str, url: &str) -> Result<String, String> {
    let track = state
        .db
        .lock()
        .get_track_by_path(track_path)
    ...
    let found = resolver().fetch_chosen(&req, url).map_err(|e| e.to_string())?;
    Ok(data_uri(&found))
}
...
pub fn preview_cover_downloads(
    state: &AppState,
    events: &Events,
    scope: &str,
    target: &str,
    ...
    let plans = resolver().resolve_many(reqs, &opts, |p| {
        events.emit(AppEvent::CoverProgress(p.clone()));
    });
...
pub fn download_covers(
    ...
    std::thread::spawn(move || {
        ...
        events.emit(AppEvent::CoverComplete(plans));
        running().store(false, Ordering::SeqCst);
    });
...
/// Empty the cover cache. `legacy_dir` is the directory the desktop app used
/// before the cache moved to a platform cache dir the phone can reach too —
/// the caller resolves it, because where an app kept its data is the app
/// frame's business.
pub fn clear_cover_cache(legacy_dir: Option<std::path::PathBuf>) -> Result<u32, String> {
...
pub fn suggest_track_names(state: &AppState, track_path: &str) -> Result<TrackNames, String> {
    ...
    let http = tunante_art::http::UreqHttp::default();
    let entries = tunante_art::tracklist::fetch(&http, system, &game);
    if entries.is_empty() {
        return Ok(TrackNames {
            file,
            subsongs,
            ...
            lengths: Vec::new(),
            named: 0,
            problem: Some(format!("No listing for \"{game}\" in the archive.")),
        });
    }
    ...
    if named == 0 {
        return Ok(TrackNames {
            ...
            named: 0,
            problem: Some(format!(
                "The archive lists {} tracks for \"{game}\" but has named \
