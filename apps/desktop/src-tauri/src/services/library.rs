//! The library itself: scanning, ratings, metadata, and opening things in the
//! system file manager. Bodies extracted verbatim from `commands/library.rs`.
...
fn spawn_rating_resolution(state: &Arc<AppState>, events: &Events) {
    let order = rating_order(state);
    if order.first() == Some(&metadata::rating_source::RatingSource::Db) {
        return;
    }
...
pub fn get_all_tracks(state: &Arc<AppState>, events: &Events) -> Result<Vec<Track>, String> {
    ...
    spawn_rating_resolution(state, events);
    Ok(tracks)
}
...
pub fn get_faved_tracks(state: &Arc<AppState>) -> Result<Vec<Track>, String> {
...
pub fn scan_folder_sync(state: &Arc<AppState>, events: &Events, path: &str) {
    ...
    log::info!(
        "Scan found {} audio files in {} (walk took {:?})",
        total,
        path,
        start_time.elapsed()
    );
...
/// Scan in a background thread. The synchronous body is [`scan_folder_sync`].
pub fn scan_folder(state: &Arc<AppState>, events: Events, path: String) {
    let state = state.clone();
    std::thread::spawn(move || {
        scan_folder_sync(&state, &events, &path);
    });
}

pub fn add_files(state: &Arc<AppState>, paths: Vec<String>) -> Result<(), String> {
...
pub fn resync_library(state: &Arc<AppState>, events: Events) {
    let state = state.clone();
    std::thread::spawn(move || {
        // Clear all tracks
        {
        ...
        for folder in &folders {
            scan_folder_sync(&state, &events, &folder.path);
        }
...
pub fn get_artwork(track_path: &str) -> Result<Option<String>, String> {
...
fn linux_open_path(path: &str, select_file: bool) -> Result<(), String> {
    ...
    log::info!(
        "linux_open_path: path={}, folder={}, select={}",
        path,
        folder.display(),
        select_file
    );
    ...
    if select_file
        && (default_fm.contains("dolphin")
            || default_fm.contains("nautilus")
            || default_fm.is_empty())
    {
    ...
    if default_fm.contains("dolphin") {
        ...
        {
            log::info!(
                "Opened folder via dolphin --new-window: {}",
                folder.display()
            );
            return Ok(());
        }
...
pub fn open_containing_folder(path: &str) -> Result<(), String> {
    ...
    log::info!(
        "open_containing_folder: path={}, folder={}",
        path,
        folder.display()
    );
