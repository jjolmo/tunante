//! Which database this app opens — the step that unified the two.
//!
...
fn resolve_in(base: &Path) -> Result<PathBuf, std::io::Error> {
    ...
    let mini_dir = base.join("tunante");
    let mini = mini_dir.join("tunante.db");
...
mod tests {
    ...
    fn without_a_desktop_database_nothing_changes() {
        ...
        assert_eq!(got, base.join("tunante/tunante.db"));
    ...
    fn a_desktop_database_is_adopted_and_mini_keys_imported_once() {
        ...
        let mini_path = base.join("tunante/tunante.db");
        ...
        mini.set_setting("mini.sleep_minutes", "30").unwrap();
        ...
        assert_eq!(
            dst.get_setting("mini.sleep_minutes").unwrap().as_deref(),
            Some("30")
        );
        assert_eq!(
