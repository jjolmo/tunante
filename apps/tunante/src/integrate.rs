...
pub fn make_desktop_entry() -> Result<(), String> {
    ...
    let entry = format!(
        "[Desktop Entry]\n\
         ...
         StartupWMClass=tunante\n",
        exe.display()
    );
    // The file's basename must equal the window's app_id (`tunante`, set in
    ...
    // A `tunante-mini.desktop` from a build before the rename would leave a
    // second menu entry pointing at a binary that no longer exists, so retire it.
    let _ = std::fs::remove_file(apps.join("tunante-mini.desktop"));
